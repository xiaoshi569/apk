...
public class MainActivity extends AppCompatActivity{
    ...
    // 支付宝调试方法 - 检查支付宝是否已安装及包名
    public void checkAlipay(View v) {
        PackageManager pm = getPackageManager();

        // 检查常见的支付宝包名
        String[] alipayPackages = {
            "com.eg.android.AlipayGphone",  // 旧版支付宝
            "com.alipay.android.app",       // 新版支付宝
            "com.alibaba.android.rimet"     // 钉钉（有时也会有支付功能）
        };

        StringBuilder result = new StringBuilder("支付宝检测结果:\n");
        boolean foundAlipay = false;

        for (String packageName : alipayPackages) {
            try {
                pm.getPackageInfo(packageName, PackageManager.GET_ACTIVITIES);
                result.append("✓ 找到: ").append(packageName).append("\n");
                foundAlipay = true;
            } catch (PackageManager.NameNotFoundException e) {
                result.append("✗ 未找到: ").append(packageName).append("\n");
            }
        }
